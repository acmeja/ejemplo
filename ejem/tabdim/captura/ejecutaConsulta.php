...
if(isset($campo) && $campo != ""){
	$campos[0]= $campo;
	$consulta = genTablaConsulta($tablaSelec,obtieneRegistrosTabla($tablaSelec,$campos,$valorCampo,$limite,$total),"../",$limite,$total);
	unset($_SESSION['campo']);
	unset($_SESSION['valorBuscar']);
}else{
	$consulta = genTablaConsulta($tablaSelec,obtieneRegistrosTabla($tablaSelec,obtieneCamposEsp($tablaSelec,"nomcolumn"),"",$limite,$total),"../",$limite,$total);
}
echo $consulta;