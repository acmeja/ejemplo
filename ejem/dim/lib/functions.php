...
/*
* Nombre: obtieneNombreTabla
* Función del módulo: regresa el nombre de la tabla correspondiente al id
* Recibe como parámetro el identificador de la tabla 
* Fecha: 30/04/2013
* Realizó: Juan Carlos Piña Moreno, Jesus Abel Vera Cruz
*/
function obtieneNombreTabla($idTabla){
	conectar();
...
function obtieneCampos($id){
	...
	$result = pg_query($query);
	$formulario = "<script type='text/javascript' src='../lib/libreria.js'></script>
	<script type='text/javascript' src='../lib/SpryAssets/SpryValidationTextField.js'></script>
	...
	<b>Ingrese los datos correspondientes:</b>
	<hr>
	<br>
	<table width=''><tr><td width='50%' align='right'><form action='../captura/alta.php' name='formAdd' method='post' target='_top'>";
	while($columna = pg_fetch_array($result)){
		if(strcmp (trim($columna['destpodts']),"boolean") != 0){//compara si es un booleano
			$formulario = $formulario."<p>".crearCampoValidacion($columna['destpodts'],$count,$columna['leycolumn'],$columna['nulcolumn'],$columna['loncolumn'],$columna['valcolumn'],'')."</p>";
		}else{
			$formulario = $formulario."<p><label class='etiqueta'>".$columna['leycolumn']."<select name='columna".$count."'>
	...
	$formulario = $formulario."<input type='hidden' name='agregar' value='si'/><input type='hidden' name='colsx'/><input type='hidden' name='nameTable'/><input type = 'submit' value='Guardar' onclick='javascript:enviaAdd(document.formAdd,$count,".'"'.$nomTabla.'"'.");' id='botonGenerado'/></form></td><td></td></tr></table>";
	$arrayResult[0] = $formulario;//formulario
	desconectar();
	//$nombreArch = "generado/auxiliar".time();
	//$arrayResult[3] = $nombreArch;
	echo $_SESSION['archAgregar'];

	$archivo = fopen($_SESSION['archAgregar'].".html","w+");
	if($archivo != ""){
		fwrite($archivo,$arrayResult[0]);
	}else{
	}
	fclose($archivo);
	return $arrayResult;
...
//Columnas a agregar de la tabla
	$txtquery = "select column_name
						  from information_schema.columns
						  where table_name = '".str_replace(" ","",$table)."'";

						  echo $txtquery;

	$querypg = pg_query($txtquery);
	...
	$i = 0;
	$larrcolumn;
	while ($rows = pg_fetch_array($querypg)) 
	{
		$larrcolumn[$i] = $rows[0];			
		$i++;
	}
	$j = count($larrcolumn)-2;

	while ($j >= 0 ) 
	{
		$txtcolumns = $txtcolumns.$larrcolumn[$j].",";
		$j--;
	}
	...
	$query = "insert into $table (".$txtcolumns.") values (";
	for($i = 0; $i < $numCol;$i++){
...
{   
 ...
 //Valida la opcion de nulos en los campos
  ...
  if(strcmp(trim($validacion),"char")==0)
  {
               ...
               <label for="columna'.$contador.'" class="etiqueta">'.$nombreCol.'</label>
...
* Realizó: Juan Carlos Piña Moreno, Jesus Abel Vera Cruz
*/
function obtieneRegistrosTabla($idTabla,$camposTabla,$valorCampo,$limite,$total){
	conectar();
	$nombreTabla ="";
	$contador = 0;
	$result = array();
	...
	$query = genQueryConsulta($nombreTabla,$camposTabla,$valorCampo,$limite,20);
...
*/
function genQueryConsulta($nombreTabla,$camposTabla,$valorBuscar,$limite,$numRegistros){
	$query = "select *";
	...
	$query = $query." limit $numRegistros offset $limite";
	return $query;
}

/*
* Nombre: numRows
* Función del módulo: obtiene el numero de registros de una consulta para el proceso de paginación
* Parámetros: recibe como entrada el identificador de la tabla a consultar, el campo de referencia para contar, el campo de referencia para bscar y el valor a buscar
* Fecha: 03/05/2013
* Realizó: Juan Carlos Piña Moreno
*/
function numRows($idTabla,$campoContar,$camposTabla,$valorBuscar){
	$query = "select $campoContar from ".obtieneNombreTabla($idTabla);
	if($valorBuscar != ""){
		$query = $query." where $camposTabla[0]::text like '%$valorBuscar%'";// con ::tex se convierte todos los campos a string para pode utilizar like
	}
	$res = pg_query($query);
...
* Realizó: Juan Carlos Piña Moreno
*/
function genTablaConsulta($idTabla,$registrosEncontrados,$action,$limite,$total){
	$registros = array();
	...
	if($limite > 0){
		$limit = $limite - 20;
		$pagAnt = "<a class='anterior' align='right'  onClick=\"cargar(".$limit.")\">Anterior</a>";
	}else{
		$pagAnt = "<a class='anterior'></a>";
	}
	if($limite < ($total-20)){
		$limit = $limite + 20;
		$pagSig = "<a class='siguiente' onClick=\"cargar(".$limit.")\">Siguiente</a>";
	}else{
		$pagSig = "<a class='siguiente'></a>";
	}
	$tabla = '<table align="center" border="0" width="100%">
			  	<tr> 
				<td colspan="'.($numCampos+1).'" align="right">'.$pagAnt.' '.$pagSig.'</td>
				</tr>
				<tr class="encabezado">';
	...
	for($i = 0;$i < count($registrosEncontrados);$i++){
		$tabla = $tabla.'<tr class="registro'.$i%2.0.'">';
	...
	$generaCombo=generaComboFiltro($idTabla,$action);
	$tabla = '<!DOCTYPE html>
				<html lang="es">
				<head>
				<meta charset="utf-8">
				...
				<body>'.$generaCombo.$tabla;
	$tabla = $tabla.'</table>
	<form name=formDelete action="../captura/elimina.php" method="post" target="_top">
		<input type="hidden" name="columna0"/>
		<input type="hidden" name="idTabla"/>
		<input type="hidden" name="nomColumna"/>
	</form>
	<form name=formEdit action="../captura/editar.php" method="post">
		<input type="hidden" name="columna0"/>
		<input type="hidden" name="idTabla"/>
		<input type="hidden" name="nomColumna"/>
	</form>
...
function generaComboFiltro($idTabla,$action){
	...
	$form = "";
	$form = $form.'<form name="formFiltro" method="post" action="'.$action.'" target="_top">
